...
`Rsassy` exposes the Rust `sassy` approximate string matcher through R's native
C API. It is useful for short-pattern approximate search over DNA, IUPAC, and
ASCII alphabets.
...
Character vectors search every pattern against every text. The `pattern_idx` and
`text_idx` columns identify the 0-based input indices.
